...
- [Development guidelines](#development-guidelines)
    - [General EPGC Core](#general-epgc-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
EPGC Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
EPGC Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/epgc-project/epgc/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_epgc
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_epgc --log_level=test_suite
$ valgrind -v --leak-check=full src/epgcd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_epgc.coverage/index.html`.
```
...
However, in open source software such as EPGC Core, where everyone uses
...
pay attention to for reviewers of EPGC Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: EPGC Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against EPGC Core so that they can be integrated
...
file descriptors it will cause problems with EPGC's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:EPGC-Project/EPGC.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `epgc-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `epgc-cli` and the GUI debug console use this table to determine how to
