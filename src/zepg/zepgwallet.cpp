...
CzEPGWallet::CzEPGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zepg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEPGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEPGSeed_deprecated()) {
                    LogPrintf("%s: Updated zEPG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zepg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zepg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEPGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEPGCount(nCountLastUsed);
    else if (!walletdb.ReadZEPGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEPGWallet::Lock()
{
    seedMaster = 0;
}

void CzEPGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEPGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEPG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEPGWallet::LoadMintPoolFromDB()
{
...
void CzEPGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEPGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEPGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zepgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEPGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEPG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zepgTracker which also adds to database
    pwalletMain->zepgTracker->Add(dMint, true);
...
void CzEPGWallet::SeedToZEPG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEPGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEPGWallet::GenerateDeterministicZEPG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEPGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEPGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEPGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
