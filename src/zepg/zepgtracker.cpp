...
CzEPGTracker::~CzEPGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEPGTracker::Init()
{
...
bool CzEPGTracker::Archive(CMintMeta& meta)
{
...
bool CzEPGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEPGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEPGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEPGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzEPGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzEPGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzEPGTracker::GetSerialHashes()
{
...
CAmount CzEPGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEPGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEPGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEPGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEPGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEPGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEPGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEPGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEPGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEPGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEPGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEPGWallet* zEPGWallet)
{
    bool iszEPGWalletInitialized = (NULL != zEPGWallet);
    CMintMeta meta;
    ...
    if (! iszEPGWalletInitialized)
        zEPGWallet = new CzEPGWallet(strWalletFile);
    meta.isSeedCorrect = zEPGWallet->CheckSeed(dMint);
    if (! iszEPGWalletInitialized)
        delete zEPGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEPGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEPGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEPGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEPGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEPGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEPGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzEPGWallet* zEPGWallet = new CzEPGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zEPGWallet);
        }
        delete zEPGWallet;
        LogPrint("zero", "%s: added %d dzepg from DB\n", __func__, listDeterministicDB.size());
    }
