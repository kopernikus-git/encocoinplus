...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current EPGC address for receiving payments to this account.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a epgc address.\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZepgSpend(nAmount, vMintsSelected, address_str);
}
...
extern UniValue DoZepgSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 6)
        throw std::runtime_error(
          "spendrawzerocoin \"serialHex\" denom \"randomnessHex\" \"priv key\" ( \"address\" \"mintTxId\" )\n"
            ...
            "5. \"address\"          (string, optional) EPGC address to spend to. If not specified, "
    ...
    return DoZepgSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
