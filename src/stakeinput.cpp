...
CZEpgStake::CZEpgStake(const libzerocoin::CoinSpend& spend)
{
...
int CZEpgStake::GetChecksumHeightFromMint()
{
...
int CZEpgStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZEpgStake::GetChecksum()
{
    return nChecksum;
}

// The zEPG block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZEpgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZEpgStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zEPG staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZEpgStake::GetUniqueness()
{
    //The unique identifier for a zEPG is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZEpgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZEpgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zEPG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZEPGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zEPG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zEPG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZEPGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zEPG output", __func__);
...
bool CZEpgStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZEpgStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzEPGTracker* zepgTracker = pwallet->zepgTracker.get();
    CMintMeta meta;
    if (!zepgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zepgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!EPG Stake
bool CEpgStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CEpgStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}
...
bool CEpgStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CEpgStake::GetUniqueness()
{
