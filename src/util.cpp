...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "epgc" is a composite category enabling all EPGC-related debug output
            if (ptrCategory->count(std::string("epgc"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\EPGC
// Windows >= Vista: C:\Users\Username\AppData\Roaming\EPGC
// Mac: ~/Library/Application Support/EPGC
// Unix: ~/.epgc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EPGC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "epgc.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty epgc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override epgc.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "epgcd.pid"));
