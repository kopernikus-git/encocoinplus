...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZepg){
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZepg)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZepg);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EPGC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no EPGC: URI
...
{
    // Convert epgc:// to epgc:
    //
    //    Cannot handle this later, because epgc:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open epgc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EPGC.lnk";
}
...
{
    return GetAutostartDir() / "epgc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a epgc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EPGC\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the epgc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add epgc app to startup item list
