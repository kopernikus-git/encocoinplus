...
#include "qt/epgc/privacywidget.h"
#include "qt/epgc/forms/ui_privacywidget.h"
#include "qt/epgc/qtutils.h"
#include "guiutil.h"
#include "qt/epgc/denomgenerationdialog.h"
#include "qt/epgc/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(EPGCGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zEPG anonymizes your EPG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zEPG or convert back to EPG"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 EPG ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zEPG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zEPG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zEPG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zEPG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zEPG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zEPG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zEPG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zEPG");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zEPG Movements"));
    ...
    // Only Convert to EPG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzEPG->setTitleClassAndText("btn-title-grey", "Total 0 zEPG");
    ui->btnTotalzEPG->setSubTitleClassAndText("text-subtitle", "Show denominations of zEPG owned.");
    ui->btnTotalzEPG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select EPG outputs to mint into zEPG.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zEPG");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzEPG, SIGNAL(clicked()), this, SLOT(onTotalZepgClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zEPG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of EPG to mint into zEPG"));
    }else{
        btnText = tr("Convert back to EPG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zEPG to convert back into EPG"));
    }
...
void PrivacyWidget::onTotalZepgClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzEPG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzEPG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zEPG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zEPG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zEPG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zEPG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZepg(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zEPG converted back to EPG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEPG </b>";
    ...
    ui->btnTotalzEPG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
