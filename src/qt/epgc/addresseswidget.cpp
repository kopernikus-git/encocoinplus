...
#include "qt/epgc/addresseswidget.h"
#include "qt/epgc/forms/ui_addresseswidget.h"
#include "qt/epgc/addresslabelrow.h"
#include "qt/epgc/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(EPGCGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a EPGC address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress epgAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(epgAdd.Get(), label.toUtf8().constData(),
                epgAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
