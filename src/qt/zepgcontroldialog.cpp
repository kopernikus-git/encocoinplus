...
#include "ui_zepgcontroldialog.h"
...
std::set<std::string> ZEpgControlDialog::setSelectedMints;
std::set<CMintMeta> ZEpgControlDialog::setMints;

bool CZEpgControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEpgControlDialog::COLUMN_DENOMINATION || column == ZEpgControlDialog::COLUMN_VERSION || column == ZEpgControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEpgControlDialog::ZEpgControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEpgControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zEPG Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZEpg->setProperty("cssClass", "text-main-purple");
    ui->labelZEpg_int->setProperty("cssClass", "text-main-purple");
...
ZEpgControlDialog::~ZEpgControlDialog()
{
    delete ui;
}

void ZEpgControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEpgControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEpgControlWidgetItem* itemDenom(new CZEpgControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEpgControlWidgetItem *itemMint = new CZEpgControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zepgTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zepgTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zEPG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zEPG seed used to mint this zEPG is not the same as currently hold in the wallet";
            else
...
void ZEpgControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEpgControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEpg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZEpgControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEpgControlDialog::GetSelectedMints()
{
...
void ZEpgControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
