...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zEPG mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zEPG mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight < 28032) {   // info->     } else if (nHeight <= last_pow_block && nHeight >= 151200) {
        nSubsidy = 1 * COIN;
    }  
...
{
	util::ThreadRename("epgc-scriptch");
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (pindexPrev && (nHeight > Params().GetConsensus().height_last_PoW)) { // pindexPrev is only null on the first block which is a version 1 block.
