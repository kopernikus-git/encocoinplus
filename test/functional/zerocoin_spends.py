...
class ZerocoinSpendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zepg_bal, epg_bal):
            zepg_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zepg_bal)
            epg_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], epg_bal)
            return zepg_bal, epg_bal
        ...
        zepg_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zepg_balance, 6666)
        ...
        zepg_balance, balance = check_balances(denom_0, zepg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zepg_balance, balance = check_balances(denom_2, zepg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zepg_balance, balance = check_balances(denom_3, zepg_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zepg",
        ...
        _, _ = check_balances(denom_1, zepg_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
